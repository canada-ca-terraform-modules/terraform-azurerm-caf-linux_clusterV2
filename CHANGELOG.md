...
- `required_version` bumped from `>= 1.9` to `>= 1.11` in `providers.tf` and `ESLZ/SRV-Linux-cluster.tf`: `override_during` on `override_resource`/`override_data` blocks (used by `tests/linux_cluster.tftest.hcl`) was only added in Terraform 1.11.0. CI's `terraform_version` pin (`.github/workflows/terraform-ci.yml`) is now `~1.11.0` to match; also fixed the pin's syntax — `setup-terraform` parses `terraform_version` using npm semver, not Terraform's own constraint syntax, so the previous `~>1.9` value was invalid and silently resolved to an older release.
