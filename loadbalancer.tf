resource "azurerm_lb" "loadbalancer" {
  # Conditional creation of the load balancer if a load balancer configuration is provided for the Linux cluster
  ...
  edge_zone = try(var.linux_vms_cluster.lb.edge_zone, null)

  # Frontend IP configuration - defines how the load balancer is exposed on the network
  dynamic "frontend_ip_configuration" {
    for_each = try(var.linux_vms_cluster.lb.frontend_ip_configuration, {})
  ...
  sku      = try(var.linux_vms_cluster.lb.sku, "Standard")
  sku_tier = try(var.linux_vms_cluster.lb.sku_tier, null)
  tags     = merge(var.tags, try(var.linux_vms_cluster.lb.tags, {}))
}
...
resource "azurerm_lb_backend_address_pool" "loadbalancer-lbbp" {
  ...
  loadbalancer_id  = azurerm_lb.loadbalancer[0].id
  synchronous_mode = try(var.linux_vms_cluster.lb.synchronous_mode, null)
  dynamic "tunnel_interface" {
    for_each = try(var.linux_vms_cluster.lb.tunnel_interfaces, {})
    content {
  ...
  virtual_network_id = try(var.linux_vms_cluster.lb.virtual_network_id, null)
}
